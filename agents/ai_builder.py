# agents/ai_builder.py
"""
AI Builder — Digital Product Creation Agent

Finds YouTube videos that actually show building/shipping a digital
product using AI tools (not just talking about AI). Sorted by view
count, filtered for recency and real depth.

Schedule: twice weekly (Tue + Fri at 10:00 UTC)
"""
...
MIN_VIEWS   = 50_000
MAX_AGE_DAYS = 540  # ~18 months — keeps results from being stale AI-tool footage
CAP_PER_RUN = 5
...
EXCLUDE_KEYWORDS = [
    ...
    "drama", "reaction", "clickbait", "shorts", "#shorts", "news update",
]

QUERIES = [
    "how I built and sold a digital product with AI",
    "AI generated digital product full tutorial",
    "build and sell AI app side hustle full guide",
    "create digital product using AI step by step",
    "AI SaaS built in a weekend full build",
    "how to build an AI tool from scratch tutorial",
]


class AIBuilder(BaseAgent):
    name = "ai_builder"
    ...
    async def _fetch_top_videos(self, client: httpx.AsyncClient, query: str, api_key: str) -> list:
        ...
        try:
            ...
            logger.error(f"[AIBuilder] YouTube search failed for '{query}': {e}")
            return []
        ...
        try:
            ...
            logger.warning(f"[AIBuilder] Stats fetch failed: {e} — skipping view filter")
            stats_map = {}
    ...
    async def _run_logic(self) -> str:
                ...
                with get_conn() as conn:
                    for video in videos:
                        ...
                        existing = conn.execute(
                            "SELECT id FROM ai_builder_videos WHERE video_id=?", (vid_id,)
                        ).fetchone()
                        if existing:
                            continue
                        ...
                        blurb = await llm_queue.submit(
                            prompt=(
                                ...
                                "First sentence: what digital product this video builds with AI.\n"
                                "Second sentence: what concrete outcome or skill the viewer walks away with.\n\n"
                                "Rules:\n"
                                "- Be specific — name the actual product or tool if clear from the title\n"
                                "- No hype words: avoid 'incredible', 'insane', 'game-changing'\n"
                                "- Do not invent features not implied by the title\n"
                            ...
                            system=(
                                ...
                                "jargon-free summaries of AI product-building videos. "
                        ...
                        conn.execute(
                            "INSERT OR IGNORE INTO ai_builder_videos"
        ...
        def video_card(v: dict) -> str:
            ...
            <div style="margin-bottom:16px;padding:14px;border:1px solid #e5e7eb;
                        border-left:4px solid #60a5fa;border-radius:8px;background:#ffffff">
                <div style="display:flex;gap:14px;align-items:flex-start">
                    ...
                    <div style="flex:1">
                        ...
                        <div style="display:flex;gap:12px;align-items:center;margin:4px 0">
                            ...
                            <span style="background:#60a5fa18;color:#60a5fa;font-size:10px;
        ...
        <div style="margin-bottom:32px">
            <h2 style="color:#1a1d27;border-bottom:3px solid #60a5fa;padding-bottom:8px;
                       margin-bottom:16px;font-size:16px">
                🛠️ AI-Built Digital Products
            </h2>
            {cards}
        </div>
        """

        html = build_email_wrapper("AI Builder — Digital Product Picks", body, "AI Builder")
        send_html_email("🛠️ AI Builder — Top Videos", html)
